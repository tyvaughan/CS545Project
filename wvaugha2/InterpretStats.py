...
# This file uses the data in userprofiles.txt to get location information over time.
...
if __name__ == '__main__':
    ...
    dateLabels = [
        ...
        'Oct. 16 2015 - Oct. 15 2016',
        'Oct. 16 2014 - Oct. 15 2015',
        'Oct. 16 2013 - Oct. 15 2014',
        'Oct. 16 2012 - Oct. 15 2013'
    ]

    # Load user information
    Users = []
    f = open('gpsLocations.txt','r')
    for line in f.readlines():
        try:
            ...
            time = int(elements[1])
            country = elements[3]
            Users.append((userId, time, country))
        except:
            print('Unable to process user: {}'.format(userId))
    f.close()

    # Parse users per continent over time.
    LocationsOverTime = []
    for i in range(5,0,-1):
        label = dateLabels[i-1]
        dateS = dates[i]
        dateF = dates[i-1]
        ...
        for user in Users:
            if(user[1] >= dateS and user[1] < dateF):
        ...
        # Store the data
        LocationsOverTime.append((label, dateS, dateF, ContCounts, ContCountryCounts))

    for year in LocationsOverTime:
        print(year[0])
        for continent, countries in year[4].items():
            print(continent)
